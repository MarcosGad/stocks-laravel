                ...
                <div class="box-header with-border">
                    ...
                    <form action="{{ route('dashboard.categories.index') }}" method="get">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <input type="text" name="search" class="form-control" placeholder="أسم القسم" value="{{ request()->search }}">
                            </div>
                        ...
                        <table class="table table-hover">
                            ...
                            @foreach ($categories as $index=>$category)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $category->name }}</td>
                                    <td @if($category->products->count() == 0) style="color:red" @endif>{{ $category->products->count() }}</td>
                                    <td><a href="{{ route('dashboard.products.index', ['category_id' => $category->id]) }}" class="btn btn-info btn-sm" target="_blank">@lang('site.related_products')</a></td>
                                    <td>
                                      @foreach ($users as $user) 
                                         @if($user->id == $category->add_by)
                                            <a href="{{ route('dashboard.reports.historyUser', $user->id) }}" target="_blank">{{strstr($user->email, '@', true)}}</a>
                                         @endif
                                      @endforeach
                                    </td>
                                    <td>
                                      @if($category->update_by != 0)
                                      @foreach ($users as $user) 
                                         @if($user->id == $category->update_by)
                                            <a href="{{ route('dashboard.reports.historyUser', $user->id) }}" target="_blank">{{strstr($user->email, '@', true)}}</a>
                                         @endif
                                      @endforeach
                                      @else
